...
public class DepthImageViewer : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		if(manager && foregroundCamera)
		{
            for (int playerNum = 0; playerNum < manager.GetUsersCount(); playerNum++)
            {
                ...
                long userId = manager.GetUserIdByIndex(playerNum);  // manager.GetPrimaryUserID();
